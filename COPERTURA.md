# Copertura del banco domande

Che cosa c'è, unità per unità. Rigenerabile con `python3 tools/copertura.py > COPERTURA.md`.

**806 domande su 2200** a regime.


---

## Chimica e propedeutica biochimica — 346/700

| | unità | domande | tipi | argomenti | quota d'esame |
|---|---|---|---|---|---|
| U1 | Atomo, legami, stati della materia, termodinamica | 150/150 | 80M + 70C | 140 | 8 |
| U2 | Miscele, soluzioni, proprietà colligative | 70/70 | 35M + 35C | 68 | 2 |
| U3 | Cinetica ed equilibrio chimico | 70/70 | 34M + 36C | 10 | 2 |
| U4 | Acidi, basi, tamponi, redox ed elettrochimica | 52/100 | 26M + 26C | 52 | 5 |
| U5 | Carbonio, idrocarburi, aromatici | 1/90 | 1M + 0C | 1 | 4 |
| U6 | Gruppi funzionali e isomerie | 1/70 | 0M + 1C | 1 | 1 |
| U7 | Amminoacidi, carboidrati, lipidi, acidi nucleici | 2/150 | 1M + 1C | 2 | 9 |

### U1 — Atomo, legami, stati della materia, termodinamica

150 domande su 150 · 80 a risposta multipla e 70 a completamento · 140 argomenti distinti · file: chimica-u1-01.json, chimica-u1-02.json, chimica-u1-03.json

- Gruppi della tavola periodica (3 domande)
- Isotopi (2 domande)
- Legame a idrogeno (2 domande)
- Legame ionico (2 domande)
- Legame metallico (2 domande)
- Numeri quantici (2 domande)
- Numero di ossidazione (2 domande)
- Orbitali molecolari (2 domande)
- Proprietà periodiche: elettronegatività (2 domande)
- Applicazione della legge di Boyle
- Applicazione delle leggi dei gas alla respirazione
- Atomo neutro
- Bioenergetica
- Calcolo con l'equazione di stato
- Calcolo della quantità di sostanza
- Calcolo delle moli
- Calore di evaporazione
- Capienza dei sottolivelli
- Capienza di un orbitale
- Classi di composti
- Classificazione degli ossidi
- Composizione dei legami multipli
- Composizione del nucleo e degli ioni
- Composti binari dell'idrogeno
- Conducibilità dei solidi ionici
- Configurazione elettronica
- Configurazione elettronica degli ioni
- Configurazione elettronica e posizione nella tavola periodica
- Confronto fra legami forti e deboli
- Costante di Avogadro
- Costituenti del nucleo
- Criterio di spontaneità
- Definizione di ione
- Definizione di mole
- Definizione di orbitale
- Diagramma di stato dell'acqua
- Diagrammi di stato
- Diagrammi di stato: acqua e anidride carbonica
- Distanza ed energia di legame
- Distribuzione di Maxwell-Boltzmann
- Ebollizione
- Ebollizione e pressione esterna
- Effetti del legame a idrogeno
- Elementi degli organismi viventi
- Elettroni esterni
- Elettroni spaiati e regola di Hund
- Energia di legame
- Energia libera di Gibbs
- Entalpia
- Entropia
- Entropia e stati di aggregazione
- Equazione di stato
- Equazione di stato dei gas perfetti
- Equilibrio e energia libera
- Equilibrio liquido-vapore
- Forma degli orbitali
- Formazione dei sali
- Formule degli idrossidi
- Formule dei composti ionici
- Forze di van der Waals
- Forze intermolecolari
- Funzioni di stato
- Geometria molecolare del carbonio sp³
- Geometria molecolare dell'acqua
- Geometria molecolare dell'ammoniaca
- Ibridazione sp
- Ibridazione sp²
- Ibridazione sp³
- Interazioni deboli nelle biomolecole
- Interazioni idrofobiche
- Interazioni idrofobiche nelle proteine
- Interpretazione cinetica della pressione
- Ioni poliatomici
- Ipotesi del modello del gas ideale
- Isotopi del carbonio
- Legame covalente dativo
- Legame covalente omopolare
- Legame covalente polare
- Legame dativo
- Legge di Boyle e meccanica respiratoria
- Legge di Charles
- Leggi dei gas
- Massa atomica relativa e abbondanza isotopica
- Massa molare
- Massa molecolare
- Mole e composizione delle molecole
- Mole e numero di Avogadro
- Momento dipolare e simmetria
- Nomenclatura degli anioni
- Nomenclatura tradizionale
- Nuclidi
- Numero di massa
- Numero di ossidazione del carbonio
- Numero di ossidazione dell'azoto
- Numero di ossidazione dell'ossigeno
- Orbitali
- Orbitali e sottolivelli
- Orbitali molecolari sigma e pi greco
- Ordinamento della tavola periodica
- Ordine di riempimento degli orbitali
- Ossidi acidi e basici
- Particelle subatomiche
- Passaggi di stato
- Polarità delle molecole
- Principio di Avogadro
- Principio di esclusione
- Proprietà dei solidi ionici
- Proprietà del legame ionico
- Proprietà magnetiche del nucleo e risonanza magnetica
- Proprietà periodiche: affinità elettronica
- Proprietà periodiche: andamento nel gruppo
- Proprietà periodiche: carattere metallico
- Proprietà periodiche: energia di ionizzazione
- Proprietà periodiche: raggio atomico
- Raggio ionico
- Reazioni esotermiche ed endotermiche
- Regola dell'ottetto
- Regola di Hund
- Regole del numero di ossidazione
- Relazione fra massa molare e massa molecolare
- Repulsione fra coppie elettroniche
- Risonanza magnetica nucleare
- Segni di ΔH e ΔS e spontaneità
- Solidi covalenti
- Solidi molecolari
- Stato aeriforme: legge di Boyle
- Struttura del ghiaccio
- Struttura dell'atomo: numero atomico e numero di massa
- Temperatura assoluta
- Tensione superficiale
- Teoria cinetica dei gas
- Teoria cinetica: natura degli urti
- Termoregolazione ed evaporazione
- Tipi di formula chimica
- Tipi di legame covalente
- Tipi di solido
- Trasformazioni termiche
- Unità di massa atomica
- Velocità molecolare e massa
- Volume molare dei gas

### U2 — Miscele, soluzioni, proprietà colligative

70 domande su 70 · 35 a risposta multipla e 35 a completamento · 68 argomenti distinti · file: chimica-u2-01.json, chimica-u2-02.json

- Diluizioni (2 domande)
- Legge di Dalton (2 domande)
- Abbassamento della tensione di vapore
- Aerosol
- Applicazioni dell'abbassamento crioscopico
- Applicazioni della legge di Henry
- Aria inspirata ed espirata
- Calcolo del fattore di van't Hoff
- Calcolo dell'abbassamento crioscopico
- Calcolo dell'innalzamento ebullioscopico
- Calcolo dell'osmolarità
- Calcolo della molarità
- Calcolo delle pressioni parziali
- Colloidi
- Componenti di una soluzione
- Comportamento cellulare in soluzione ipertonica
- Comportamento dei globuli rossi in soluzione ipotonica
- Composizione dell'aria
- Definizione di osmosi
- Definizione di pressione osmotica
- Definizione di proprietà colligative
- Diffusione
- Direzione del flusso osmotico
- Elettroliti
- Elettroliti forti e deboli
- Elettroliti nei fluidi biologici
- Emolisi
- Fattore di van't Hoff
- Fattore di van't Hoff e abbassamento crioscopico
- Formula della pressione osmotica
- Frazione molare
- Il concetto di equivalente
- Innalzamento ebullioscopico
- Interpretazione dell'abbassamento della tensione di vapore
- L'acqua come solvente polare
- Legge di Henry
- Legge di Raoult
- Membrane
- Miscele omogenee ed eterogenee
- Molalità
- Molarità
- Molarità e molalità a confronto
- Natura delle proprietà colligative
- Non elettroliti
- Osmolarità
- Osmolarità e osmolalità
- Percentuale peso su peso
- Percentuale peso su volume
- Preparazione di una soluzione a titolo noto
- Pressione oncotica
- Pressione parziale
- Proprietà colligative e numero di particelle
- Proprietà colligative: pressione osmotica e fattore di van't Hoff
- Proprietà dei colloidi
- Saturazione
- Solubilità dei gas e temperatura
- Solubilità dei gas: rilevanza biomedica
- Solubilità dei solidi e temperatura
- Solubilità e polarità
- Soluzione fisiologica
- Soluzione glucosata
- Soluzioni non ideali e attività
- Soluzioni solide, liquide e gassose
- Solvatazione
- Sospensioni
- Squilibri osmotici: edema
- Tonicità
- Unità percentuali di concentrazione

### U3 — Cinetica ed equilibrio chimico

70 domande su 70 · 34 a risposta multipla e 36 a completamento · 10 argomenti distinti · file: chimica-u3-01.json, chimica-u3-02.json, chimica-u3-03.json

- arrhenius e stato di transizione (10 domande)
- costante di equilibrio (10 domande)
- cinetica: velocità e fattori (9 domande)
- equilibrio mobile e stato stazionario (8 domande)
- ordine e molecolarità (8 domande)
- reazioni e bilanciamento (7 domande)
- catalisi ed enzimi (6 domande)
- energia libera ed equilibrio (6 domande)
- solubilità e ione comune (5 domande)
- Cinetica: catalizzatori ed energia di attivazione

### U4 — Acidi, basi, tamponi, redox ed elettrochimica

52 domande su 100 · 26 a risposta multipla e 26 a completamento · 52 argomenti distinti · file: chimica-u4-01.json, chimica-u4-02.json

- Acidi poliprotici: dissociazioni successive
- Alterazioni dell'equilibrio acido-base
- Autoprotolisi dell'acqua
- Autoprotolisi dell'acqua: dipendenza di Kw dalla temperatura
- Bilanciamento per semireazioni
- Calcolo del numero di ossidazione in uno ione poliatomico
- Capacità tamponante
- Cella galvanica: elettrodi e polarità
- Classificazione degli acidi per numero di protoni cedibili
- Comportamento acido o basico dei sali in acqua
- Composizione di una soluzione tampone
- Condizione di equimolarità in un tampone
- Condizione di massimo potere tamponante
- Confronto fra le forze di due acidi deboli
- Coppie acido-base coniugate
- Costante di dissociazione basica
- Definizione di acido secondo Brønsted e Lowry
- Definizione di catodo
- Definizione di ossidazione
- Definizione di soluzione tampone
- Effetto della diluizione sul pH di un acido forte
- Elettrochimica: elettrodi di una cella galvanica
- Elettrodo di riferimento dei potenziali standard
- Equazione di Henderson-Hasselbalch: rapporto base/acido
- Equazione di Henderson-Hasselbalch: tampone equimolare
- Equazione di Nernst
- Identificazione dell'agente ossidante
- Idrolisi del catione di un sale
- Indicatori di pH: meccanismo del viraggio
- Intervallo di viraggio di un indicatore
- Meccanismo d'azione di un tampone
- Nome dell'equazione dei tamponi
- Reazione fra un acido e una base
- Reazioni di dismutazione
- Reazioni redox non enzimatiche dell'ossigeno
- Regola di somma dei numeri di ossidazione
- Relazione fra Ka e Kb di una coppia coniugata
- Relazione fra energia libera e differenza di potenziale
- Relazione fra pH e pOH
- Ruolo delle due componenti di un tampone
- Significato del pKa
- Solubilità e pH: acido urico e urati
- Soluzioni tampone: equazione di Henderson-Hasselbalch
- Somma di pH e pOH
- Specie anfotere
- Tampone fosfato
- Tamponi del sangue
- Teoria di Brønsted e Lowry: individuazione dell'acido e della base
- Teoria di Lewis: definizione di acido
- pH di una soluzione di acido debole
- pH di una soluzione di acido forte
- pH di una soluzione di base forte

### U5 — Carbonio, idrocarburi, aromatici

1 domande su 90 · 1 a risposta multipla e 0 a completamento · 1 argomenti distinti · file: chimica-u5-01.json

- Idrocarburi aromatici: struttura e reattività del benzene

### U6 — Gruppi funzionali e isomerie

1 domande su 70 · 0 a risposta multipla e 1 a completamento · 1 argomenti distinti · file: chimica-u6-01.json

- Alcoli: reazioni di ossidazione

### U7 — Amminoacidi, carboidrati, lipidi, acidi nucleici

2 domande su 150 · 1 a risposta multipla e 1 a completamento · 2 argomenti distinti · file: chimica-u7-01.json

- Acidi nucleici: legame fra nucleotidi
- Proteine: natura del legame peptidico

---

## Fisica — 270/680

| | unità | domande | tipi | argomenti | quota d'esame |
|---|---|---|---|---|---|
| U1 | Introduzione ai metodi della fisica | 80/70 | 40M + 40C | 76 | 4 |
| U2 | Meccanica | 130/130 | 65M + 65C | 122 | 5 |
| U3 | Meccanica dei fluidi | 60/110 | 30M + 30C | 59 | 5 |
| U4 | Onde meccaniche | 0/70 | 0M + 0C | 0 | 4 |
| U5 | Termodinamica | 0/100 | 0M + 0C | 0 | 5 |
| U6 | Elettricità e magnetismo | 0/120 | 0M + 0C | 0 | 5 |
| U7 | Fisica delle radiazioni | 0/80 | 0M + 0C | 0 | 3 |

### U1 — Introduzione ai metodi della fisica

80 domande su 70 · 40 a risposta multipla e 40 a completamento · 76 argomenti distinti · file: fisica-u1-01.json, fisica-u1-02.json, fisica-u1-03.json

- Analisi dimensionale (2 domande)
- Grandezze estensive (2 domande)
- Notazione scientifica (2 domande)
- Ordine di grandezza (2 domande)
- Annullamento del prodotto vettoriale
- Componenti cartesiane di un vettore
- Componenti di un vettore
- Conversione delle unità di densità
- Conversione delle unità di tempo
- Conversione di unità di superficie
- Conversione di unità di velocità
- Conversione di unità di volume
- Conversione fra gradi e radianti
- Definizione di coseno
- Definizione di seno
- Definizione di tangente
- Differenza fra vettori
- Direzione del prodotto vettoriale
- Direzione di un vettore ricavata dalle componenti
- Divisione in notazione scientifica
- Equazione dimensionale della potenza
- Equazione dimensionale della pressione
- Equivalenza fra metro cubo e litro
- Grafici: proporzionalità diretta
- Grafici: proporzionalità inversa
- Grafico della proporzionalità quadratica
- Grandezza fondamentale misurata in kelvin
- Grandezze adimensionali
- Grandezze fondamentali del Sistema Internazionale
- Grandezze intensive
- Grandezze intensive ed estensive
- Grandezze scalari
- Grandezze scalari e vettoriali
- Grandezze vettoriali
- Lato del triangolo rettangolo
- Lettura di un grafico lineare
- Modulo del prodotto vettoriale
- Modulo della somma di due vettori
- Modulo di un vettore
- Modulo di un vettore dalle componenti
- Multipli e sottomultipli
- Operazioni in notazione scientifica
- Prefissi del Sistema Internazionale
- Prefisso giga
- Prefisso milli
- Prefisso pico
- Prodotto di un vettore per uno scalare
- Prodotto scalare
- Prodotto scalare calcolato per componenti
- Prodotto scalare di vettori perpendicolari
- Prodotto scalare e angolo ottuso
- Prodotto scalare e vettori perpendicolari
- Prodotto vettoriale
- Proporzionalità quadratica
- Regola del parallelogramma
- Relazione fondamentale della trigonometria
- Scomposizione di un vettore lungo gli assi
- Somma di due vettori con angolo qualsiasi
- Somma di vettori per componenti
- Somma di vettori perpendicolari
- Somma in notazione scientifica
- Trigonometria del triangolo rettangolo
- Unità derivate espresse in unità fondamentali
- Unità di misura degli angoli
- Unità di misura dell'energia
- Unità di misura della forza
- Unità di misura della potenza
- Unità di misura della quantità di sostanza
- Unità fondamentale del tempo
- Unità fondamentale dell'intensità di corrente
- Unità fondamentale dell'intensità luminosa
- Valori del coseno agli angoli notevoli
- Versore
- Vettori complanari
- Vettori concordi
- Vettori discordi

### U2 — Meccanica

130 domande su 130 · 65 a risposta multipla e 65 a completamento · 122 argomenti distinti · file: fisica-u2-01.json, fisica-u2-02.json, fisica-u2-03.json

- Accelerazione centripeta (2 domande)
- Forze conservative (2 domande)
- Legge di Hooke (2 domande)
- Moto rettilineo uniforme (2 domande)
- Moto uniformemente accelerato (2 domande)
- Spazio di arresto (2 domande)
- Urto perfettamente anelastico (2 domande)
- Velocità media (2 domande)
- Accelerazione
- Accelerazione di gravità
- Accelerazione tangenziale
- Area sottesa dal grafico velocità-tempo
- Attrito dinamico
- Attrito radente dinamico
- Attrito statico
- Baricentro
- Braccio di una forza
- Caduta dei gravi nel vuoto
- Caduta libera: velocità
- Calcolo dell'energia cinetica
- Calcolo dell'energia potenziale elastica
- Calcolo dell'impulso
- Carrucola
- Carrucola fissa
- Centro di massa
- Centro di massa di due corpi
- Centro di massa di un sistema isolato
- Coefficiente di attrito
- Comportamento dell'attrito statico
- Condizione di scivolamento sul piano inclinato
- Condizioni di equilibrio del corpo rigido
- Conservazione dell'energia con una molla
- Conservazione dell'energia meccanica
- Conservazione dell'energia su un piano inclinato
- Conservazione della quantità di moto: rinculo
- Corpi collegati
- Corpo rigido
- Costante elastica
- Dipendenza dello spazio di arresto dalla velocità
- Energia cinetica
- Energia cinetica e velocità
- Energia meccanica
- Energia potenziale
- Energia potenziale elastica
- Energia potenziale gravitazionale
- Equilibrio del corpo rigido
- Equilibrio di un corpo
- Equilibrio di una leva
- Equilibrio rotazionale
- Equilibrio traslazionale
- Equilibrio traslazionale e tensione
- Esplosione
- Forza gravitazionale
- Forza normale
- Forza peso
- Forza peso: calcolo
- Forza risultante
- Forze dissipative
- Fulcro
- Gittata
- Grafico spazio-tempo del moto rettilineo uniforme
- Grafico velocità-tempo
- Impulso
- Indipendenza dei moti nel moto parabolico
- Lavoro della forza centripeta
- Lavoro della forza di attrito
- Lavoro di una forza
- Legge della velocità nel moto uniformemente accelerato
- Legge di gravitazione universale
- Legge oraria
- Legge oraria del moto uniformemente accelerato
- Leva vantaggiosa
- Leve di primo genere
- Leve di primo genere nel corpo umano
- Leve di secondo genere nel corpo umano
- Leve di terzo genere
- Leve nel corpo umano
- Leve svantaggiose
- Massa e peso
- Massa inerziale
- Momento di una forza
- Momento di una forza non perpendicolare
- Momento risultante e rotazione
- Momento torcente
- Moto circolare non uniforme
- Moto circolare uniforme
- Moto ritardato
- Moto verticale verso l'alto
- Piano inclinato
- Piano inclinato: componente del peso
- Potenza
- Potenza come prodotto di forza e velocità
- Potenza media
- Potenza media e potenza istantanea
- Primo principio della dinamica
- Principio d'inerzia
- Principio di conservazione della quantità di moto
- Quantità di moto
- Resistenza di una leva
- Rinculo
- Seconda legge della dinamica
- Segno del lavoro
- Segno del lavoro della forza peso
- Sistema di riferimento inerziale
- Sistema isolato
- Tempo di caduta libera
- Tensione
- Teorema dell'energia cinetica
- Teorema dell'energia cinetica applicato alla frenata
- Teorema dell'impulso
- Terzo principio della dinamica
- Traiettoria
- Traiettoria del moto parabolico
- Urto
- Urto anelastico fra corpi entrambi in moto
- Urto elastico
- Urto elastico e anelastico a confronto
- Velocità angolare
- Velocità istantanea
- Velocità nel moto circolare uniforme
- Velocità nel punto più alto
- Vettore spostamento

### U3 — Meccanica dei fluidi

60 domande su 110 · 30 a risposta multipla e 30 a completamento · 59 argomenti distinti · file: fisica-u3-02.json, fisica-u3-03.json

- Unità di misura della viscosità (2 domande)
- Barometro con un liquido diverso dal mercurio
- Bernoulli applicato alla stenosi
- Bernoulli: termine di quota in un condotto a sezione costante
- Calcolo della massa a partire dalla densità
- Calcolo della pressione esercitata da un corpo appoggiato
- Condizione di aderenza alla parete
- Condizione di affondamento
- Conservazione della portata
- Continuità: dipendenza della velocità dal raggio
- Conversione delle unità di densità
- Conversione delle unità di portata
- Conversione fra unità di pressione
- Criterio per distinguere moto laminare e turbolento
- Definizione del pascal
- Definizione di fluido ideale
- Densità relativa
- Enunciato del principio di Archimede
- Enunciato del principio di Pascal
- Equazione di continuità e velocità del sangue nei capillari
- Equilibrio indifferente di un corpo immerso
- Forma del menisco nei liquidi che bagnano
- Galleggiamento: carico massimo sostenibile
- Gradiente di velocità
- Il torr come unità di pressione
- Legge di Poiseuille: dipendenza da raggio e lunghezza
- Legge di Poiseuille: dipendenza dalla viscosità
- Legge di Stevino: differenza di pressione fra due quote
- Linee di flusso
- Natura scalare della pressione
- Numero di Reynolds
- Paradosso idrostatico
- Peso apparente e spinta di Archimede
- Portata di massa
- Pressione assoluta e pressione relativa
- Pressione assoluta in profondità
- Pressione relativa
- Profilo delle velocità nel moto laminare
- Relazione fra velocità e pressione in un condotto orizzontale
- Resistenza idraulica di un condotto
- Significato fisico del teorema di Bernoulli
- Spazio sovrastante la colonna nell'esperimento di Torricelli
- Stati di aggregazione compresi fra i fluidi
- Strumento di misura della pressione arteriosa
- Strumento di misura della pressione atmosferica
- Superfici isobare in un liquido in quiete
- Tensione superficiale e surfactante polmonare
- Teorema di Bernoulli: calcolo della differenza di pressione
- Termini dell'equazione di Bernoulli
- Terminologia: dilatazione di un vaso
- Terminologia: restringimento di un vaso
- Torchio idraulico: spostamenti dei pistoni
- Tubo di flusso
- Unità di misura della portata
- Unità di misura della pressione
- Unità di misura della tensione superficiale
- Unità pratiche di pressione: il bar
- Vasi comunicanti
- Viscosità e temperatura

### Ancora vuote

U4, U5, U6, U7.


---

## Biologia — 190/820

| | unità | domande | tipi | argomenti | quota d'esame |
|---|---|---|---|---|---|
| U1 | Le basi dell'organizzazione biologica e molecolare della vita | 80/80 | 40M + 40C | 80 | 3 |
| U2 | I meccanismi cellulari di trasmissione e controllo dell'informazione genetica e epigenetica | 60/60 | 30M + 30C | 60 | 2 |
| U3 | Il flusso dell'informazione | 50/180 | 25M + 25C | 50 | 7 |
| U4 | I meccanismi cellulari di trasmissione e controllo dei caratteri selvatici e mutati | 0/110 | 0M + 0C | 0 | 4 |
| U5 | Le strutture cellulari: biogenesi, morfologia e funzioni | 0/200 | 0M + 0C | 0 | 8 |
| U6 | La cellula e l'ambiente, la segnalazione cellulare e la trasduzione del segnale | 0/100 | 0M + 0C | 0 | 4 |
| U7 | Il controllo della proliferazione e della sopravvivenza cellulare | 0/90 | 0M + 0C | 0 | 3 |

### U1 — Le basi dell'organizzazione biologica e molecolare della vita

80 domande su 80 · 40 a risposta multipla e 40 a completamento · 80 argomenti distinti · file: biologia-u1-01.json, biologia-u1-02.json

- Anabolismo
- Anabolismo, catabolismo e reazioni di condensazione
- Appaiamento delle basi
- Appaiamento guanina-citosina
- Appendici della cellula procariotica
- Basi puriniche
- Ciclo di un retrovirus
- Ciclo lisogenico di un virus batterico
- Ciò che distingue i venti amminoacidi
- Classi di enzimi e reazioni opposte
- Compartimentazione della cellula eucariotica
- Composizione della parete batterica
- Differenze fra DNA e RNA
- Entrata di un virus nella cellula animale
- Enzimi dei retrovirus
- Eubatteri e archeobatteri
- Gli acidi grassi insaturi
- I domini proteici
- I fosfolipidi e le membrane
- I plasmidi
- I polisaccaridi del glucosio
- I ribosomi dei procarioti
- I tre domini della vita
- I tre domini e la classificazione dei viventi
- Idrolisi e degradazione dei polimeri
- Il capside virale
- Il ciclo litico
- Il colesterolo
- Il genoma dei virus
- Il legame fra gli amminoacidi
- Il legame fra i monosaccaridi
- Il legame peptidico
- Il modello di Watson e Crick
- Il nucleoide procariotico
- Il profago
- Il saccarosio
- Il sistema delle endomembrane
- Il sito attivo degli enzimi
- L'RNA messaggero
- L'albero della vita
- L'involucro membranoso dei virus
- La capsula batterica
- La catalisi enzimatica
- La colorazione di Gram
- La coniugazione batterica
- La membrana esterna dei Gram-negativi
- La parete dei Gram-positivi
- La pluricellularità
- La teoria cellulare
- Le due facce del metabolismo
- Le subunità del capside
- Le unità degli acidi nucleici
- Liberazione del genoma virale
- Lo scheletro degli acidi nucleici
- Modificazioni post-traduzionali: aggiunta di lipidi
- Modificazioni post-traduzionali: la fosforilazione
- Modificazioni post-traduzionali: la glicosilazione
- Modificazioni post-traduzionali: ubiquitinazione
- Molecole anfipatiche
- Motilità batterica
- Natura dei virus
- Omeostasi
- Origine dei mitocondri
- Parete dei batteri Gram-negativi
- Perdita della conformazione proteica
- Proprietà fondamentali della materia vivente
- Prove dell'origine endosimbiotica dei mitocondri
- RNA non codificanti
- Riserva di glucosio negli animali
- Specificità enzimatica e adattamento indotto
- Struttura quaternaria
- Struttura secondaria delle proteine
- Struttura terziaria delle proteine
- Strutture assenti nella cellula procariotica
- Trasferimento genico orizzontale: la trasduzione
- Trasferimento genico orizzontale: la trasformazione
- Trigliceridi
- Uscita di un virus animale dalla cellula
- Virus a RNA a polarità negativa
- Virus dei batteri

### U2 — I meccanismi cellulari di trasmissione e controllo dell'informazione genetica e epigenetica

60 domande su 60 · 30 a risposta multipla e 30 a completamento · 60 argomenti distinti · file: biologia-u2-01.json, biologia-u2-02.json

- Acetilazione degli istoni
- Bersaglio della metilazione del DNA
- Che cos'è una modificazione epigenetica
- Compattazione della cromatina
- Composizione del genoma umano
- Composizione del nucleosoma
- Condensazione dei cromosomi
- Copie geniche non funzionanti
- Cromatina condensata
- Cromatina trascrizionalmente attiva
- DNA satellite centromerico
- Dalla fibra da 11 nm a quella da 30 nm
- Dimensioni del genoma umano
- Elementi mobili del DNA
- Enzima dei trasposoni a DNA
- Enzimi che chiudono la cromatina
- Enzimi della metilazione del DNA
- Epigenetica
- Eterocromatina facoltativa
- Eucromatina ed eterocromatina
- Famiglie geniche
- Funzione dei telomeri
- Funzione del centromero
- Geni per gli RNA ribosomiali
- I retrovirus endogeni
- I telomeri
- Il DNA fra due nucleosomi
- Il DNA telomerico
- Il centromero
- Il cinetocore
- Il problema della compattazione
- Inattivazione del cromosoma X
- Interazione fra istoni e DNA
- Isole CpG
- Istone deacetilasi
- L'istone H1
- L'istone di collegamento
- L'unità strutturale della cromatina
- LINE e SINE
- La base metilata
- La fibra da 11 nm
- Le code istoniche
- Le condensine
- Le proteine della cromatina
- Le sequenze Alu
- Livelli superiori di compattazione
- Metilazione dei promotori e silenziamento
- Metilazione del DNA
- Microsatelliti
- Microsatelliti e minisatelliti
- Minisatelliti
- Modificazioni delle code istoniche
- Morfologia dei cromosomi e posizione del centromero
- Nucleosoma e istone H1
- Organizzazione minimale di un cromosoma eucariotico
- Pseudogeni
- Rimodellamento della cromatina
- SINE
- Sequenze a copia singola
- Varianti istoniche e identità del centromero

### U3 — Il flusso dell'informazione

50 domande su 180 · 25 a risposta multipla e 25 a completamento · 50 argomenti distinti · file: biologia-u3-03.json

- Allungamento della trascrizione
- Aminoacil-tRNA
- Antiparallelismo
- Associazione delle subunità ribosomiali
- Bolla di replicazione
- Chaperonine
- Codone
- Codone AUG
- Complesso d'inizio della replicazione
- Controllo negativo e positivo
- Controllo post-trascrizionale
- Controllo traduzionale
- Deubiquitinasi
- Dimensione del codone
- Direzione dei frammenti di Okazaki
- Editing per deaminazione
- Energia della traduzione
- Esiti del silenziamento mediato da RISC
- Fattori generali: terminologia
- Fattori richiesti dalle RNA polimerasi eucariotiche
- Fedeltà della replicazione
- Fedeltà della traduzione
- Filamento codificante
- Filamento in ritardo
- Maturazione dell'RNA
- Operone lac: la permeasi
- Operone lac: massima espressione
- Operoni inducibili
- Origine dell'informazione per il ripiegamento
- Polarità della catena polipeptidica
- Prodotti dello splicing
- Prodotto della polimerizzazione
- Promotore prossimale
- Proteine simili all'ubiquitina
- Punto di ramificazione
- Replicazione e trascrizione a confronto
- Ruolo della DNA ligasi
- Sede della maturazione degli rRNA
- Sede della maturazione dell'mRNA
- Sequenze consenso degli introni
- Siti del ribosoma: sito E
- Sito di inizio della trascrizione
- Sostituzioni in terza posizione
- TBP
- Telomerasi e proliferazione illimitata
- Telomero
- Trascritto primario
- mRNA monocistronico
- microRNA
- snRNA

### Ancora vuote

U4, U5, U6, U7.

